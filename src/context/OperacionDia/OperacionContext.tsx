...
function operacionReducer(state: SimulationState, action: SimulationAction): SimulationState {
  switch (action.type) {
...
const initialState: SimulationState = {
  isPlaying: true, // OperacionesDiaDia siempre está activo
  vehicles: [],
  speed: 1, // Velocidad de tiempo real
  ends: false,
  startTime: new Date(), // Fecha y hora actuales
  currentTime: new Date(), // Fecha y hora actuales
  endTime: new Date('2099-12-31T23:59:59Z'), // Fecha muy lejana
  trucksInMotion: 0,
...
export function OperacionProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const updateInterval = setInterval(() => {
      const newTime = new Date(); // Tiempo real
      ...
      const updatedVehicles = state.vehicles.map((vehicle) => {
        ...
        for (let i = 0; i < arrivalTimes.length; i++) {
          ...
          if (newTime >= arrivalTime && newTime < departureTime) {
            ...
            const maintenanceEndTime = new Date(
              maintenanceStartTime.getTime() + maintenanceDuration
            );
        ...
        if (currentSegmentIndex !== -1) {
          ...
          const progress = Math.max(
            0,
            Math.min(1, currentSegmentTime / totalSegmentTime)
          );

          // Si el vehículo no ha alcanzado el final del segmento, actualizar posición
          if (progress < 1) {
            const startCoords =
              locationCoordinates[ruta.tramos[currentSegmentIndex].origen.codigo];
            const endCoords =
              locationCoordinates[ruta.tramos[currentSegmentIndex].destino.codigo];
      ...
      state.vehicles.forEach((vehicle) => {
        vehicle.ruta.pedidos.forEach((pedido) => {
          if (pedido.fechaLlegada) {
            ...
            if (
              arrivalTime <= newTime &&
              !state.processedOrderIds.includes(pedido.idPedido)
            ) {
              arrivedOrders.push({
                order: pedido,
      ...
      const updatedOffices = state.offices.map((office) => {
        const updatedOffice = {
          ...office,
          currentOrders: [...(office.currentOrders ?? [])],
        };
        ...
        updatedOffice.currentOrders = updatedOffice.currentOrders.filter(
          (currentOrder) => {
            const timeInOffice =
              newTime.getTime() - currentOrder.arrivalTime.getTime();
