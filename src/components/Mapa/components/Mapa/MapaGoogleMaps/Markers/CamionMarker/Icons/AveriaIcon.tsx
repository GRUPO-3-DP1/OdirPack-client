import React, { SVGProps } from 'react';


type IconSize = 'tiny' | 'small' | 'medium' | 'large';

interface IconComponentProps extends SVGProps<SVGSVGElement> {
  size?: IconSize;
}

const sizeMap: Record<IconSize, { width: string; height: string; }> = {
  tiny: { width: '12px', height: '12px' },
  small: { width: '16px', height: '16px' },
  medium: { width: '24px', height: '24px' },
  large: { width: '32px', height: '32px' }
};

const AveriaIcon: React.FC<IconComponentProps> = ({
  size = 'medium',
  width,
  height,
  style,
  ...props
}) => {
  const sizeStyles = sizeMap[size];

  return (
    <svg
      version="1.1"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      viewBox="-184.32 -184.32 880.64 880.64"
      width={width || sizeStyles.width}
      height={height || sizeStyles.height}
      style={style}
      {...props}
    >
      <g transform="translate(12.8,12.8), scale(0.95)">
        <rect
          x="-184.32"
          y="-184.32"
          width="880.64"
          height="880.64"
          rx="440.32"
          fill="#bd002f"
        ></rect>
      </g>
      <g>
        <path
          fill="#ffffff"
          d="M459.957,203.4c42.547-38.609,49.656-82.484,40.141-119.469c-0.281-2.938-0.984-5.406-3.547-7.266
            l-8.563-7.016c-1.484-1.375-3.484-2.063-5.484-1.859c-2.016,0.188-3.844,1.234-5.031,2.859l-49.25,64.031
            c-1.375,1.891-3.594,2.969-5.922,2.891l-17.875,1.313c-1.531-0.047-3.016-0.594-4.219-1.563l-34.531-29.266
            c-1.406-1.141-2.328-2.766-2.563-4.563l-2.141-16.188c-0.25-1.781,0.203-3.594,1.266-5.047l46.109-62.641
            c2.094-2.891,1.688-6.875-0.906-9.297l-11.188-8.734c-2.188-2.047-4.672-1.75-8.063-1.109
            c-31.844,6.297-86.219,37.125-100.016,79.75c-12.156,37.516-7.922,63.969-7.922,63.969c0,21.141-6.953,41.516-15.5,50.078
            L24.504,424.916c-0.469,0.438-0.922,0.859-1.375,1.313c-19.844,19.844-19.813,52.063-0.641,71.219
            c19.172,19.172,51.859,19.688,71.703-0.172c0.922-0.922,1.813-1.875,2.641-2.859l231.672-250.438
            C357.004,218.619,413.426,245.65,459.957,203.4z"
        ></path>
      </g>
    </svg>
  );
};

export default AveriaIcon;
